...
class Analisis_Visual_Encoding:
    ...
    def subplot_cat(self, plot_type = "bar", tamanio = (15,12), estimador = "mean", paleta = "mako"):
        filas = math.ceil(len(self.lista_variables_categorias)/2)
        fig, axes = plt.subplots(nrows = filas, ncols = 2, figsize = tamanio)
        axes = axes.flat

        for i, col in enumerate(self.lista_variables_categorias):
            if plot_type.lower() == "bar":
                sns.barplot(data = self.dataframe, y = col, x = self.variable_respuesta, ax = axes[i], estimator=estimador, palette=paleta)
            elif plot_type.lower() == "box":
                sns.boxplot(data = self.dataframe, y = col, x = self.variable_respuesta, ax = axes[i], palette=paleta)

        if len(self.lista_variables_categorias) % 2 != 0:
            plt.delaxes(axes[-1])
